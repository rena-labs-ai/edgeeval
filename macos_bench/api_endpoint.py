...
class OllamaEndPoint(APIEndPoint):
    """The backend of sending HTTP requests to Ollama API."""

    def __init__(
        self,
        host: str = "localhost",
        port: int = 11434,  # Default Ollama port
        timeout: Optional[float] = None,
        include_server_metrics: bool = True
    ):
        super().__init__(include_server_metrics=include_server_metrics)
        self.host = host
        self.port = port
        ...
        self.headers = {"Content-Type": "application/json"}
        self.client = None
        logger.info(f"Initialized Ollama endpoint at {self.url}")

    async def __aenter__(self) -> Self:
        import aiohttp  # pylint: disable=import-outside-toplevel,import-error
        self.client = aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(self.timeout))
        logger.info("Created aiohttp client session")
        return self
    ...
    async def __call__(self, request_record: RequestRecord) -> RequestRecord:
        ...
        model_name = request_record.chat_cmpl.model
        # Remove "ollama/" prefix if present
            
        # Convert messages to Ollama format
        messages = []
        for msg in request_record.chat_cmpl.messages:
            if isinstance(msg.content, str):
                messages.append({"role": msg.role, "content": msg.content})
            else:
                logger.warning(f"Skipping non-string message content: {msg.content}")
                continue
                
        if not messages:
            error_msg = "No valid messages found in request"
        ...
        try:
            ...
            async with self.client.post(self.url, json=payload, headers=self.headers) as response:
                ...
                async for chunk in response.content:
                    ...
                    try:
                        ...
                        logger.debug("Received chunk: %s", json.dumps(data, indent=2))
                        
                        if "message" in data:
                            content = data["message"].get("content", "")
                            if content and not time_to_first_token_s:
                                time_to_first_token_s = time.monotonic() - start_time
                                first_chunk_output_str = content
                                logger.info("First token received after %.2fs", time_to_first_token_s)
                            generated_text += content
                        ...
                        if data.get("done", False):
                            ...
                            logger.info("Token counts - Prompt: %d, Total: %d", prompt_tokens, total_tokens)
                            
                    except json.JSONDecodeError as e:
                        logger.warning("Failed to parse chunk: %s, error: %s", chunk, e)
                        continue
                        
        except Exception as e:
            error_msg = f"API endpoint errored when sending request: {str(e)}\n{traceback.format_exc()}"
            logger.error(error_msg)
            finish_time = time.monotonic()
            request_record.output_str = generated_text
            request_record.first_chunk_output_str = first_chunk_output_str
            request_record.metrics = Metrics(
                ...
                output_tokens=total_tokens - prompt_tokens if total_tokens > 0 else 0,
                time_to_first_token_s=time_to_first_token_s,
                exec_feature=request_record.metrics.exec_feature,
            )
            request_record.error_msg = error_msg
        ...
        logger.info("Request completed in %.2fs, success: %s", finish_time - start_time, success)
        if generated_text:
            logger.debug("Generated text: %s...", generated_text[:100])
            
        # Calculate output tokens as the difference between total and prompt tokens
        output_tokens = total_tokens - prompt_tokens if total_tokens > 0 else 0
        logger.info("Final token counts - Input: %d, Output: %d, Total: %d", 
        ...
        logger.info("Final metrics: %s", json.dumps({
            ...
            "input_tokens": prompt_tokens,
            "output_tokens": output_tokens,
            "time_to_first_token": time_to_first_token_s
        }, indent=2))
...
def create_api_endpoint(args: argparse.Namespace) -> APIEndPoint:
    ...
    if args.api_endpoint == "ollama":
        return OllamaEndPoint(args.host, args.port, args.timeout, args.include_server_metrics)
