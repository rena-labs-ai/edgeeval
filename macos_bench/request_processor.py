...
class MetricAnalyzer(RequestProcessor):  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, tokenizer: Optional[AutoTokenizer] = None) -> None:
        ...
        for request_record in request_records:
            ...
            if metrics.output_tokens is not None and metrics.input_tokens is not None:
                ...
                if self.tokenizer:
                    ...
                    # For Ollama without tokenizer, estimate first chunk tokens
                    # This is a rough estimate - we could improve this if needed
                    first_chunk_output_tokens = max(1, metrics.output_tokens // 10)
            else:
                # Fall back to tokenizer-based counting
                if not self.tokenizer:
                    logger.warning("No tokenizer provided and no token counts from API")
                    continue
